    ...
    *,
    ...
    if run_by_chunk:
        if not chunk_outputs:
            raise ValueError("RUN_BY_CHUNK is enabled, but no chunk outputs were found.")

        combined_root = Path(runs_base_dir) / "combined_final"
        combined_root.mkdir(parents=True, exist_ok=True)

        df_orig = _read_csv(Path(csv_path))
        ...
        df_erosion = _concat_frames(erosion_frames)
    ...
    df_orig = preview_df_world.copy() if preview_df_world is not None else _read_csv(Path(csv_path))
    ...
    return {
        ...
        "df_erosion": _read_csv(erosion_path),
